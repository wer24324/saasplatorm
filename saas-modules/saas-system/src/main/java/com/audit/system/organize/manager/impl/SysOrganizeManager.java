...
import com.audit.system.organize.manager.ISysOrganizeManager;
import com.audit.system.organize.manager.ISysPostManager;
import com.audit.system.organize.mapper.merge.SysOrganizeRoleMergeMapper;
import com.audit.system.organize.mapper.merge.SysRoleDeptMergeMapper;
import com.audit.system.organize.mapper.merge.SysRolePostMergeMapper;
import com.audit.system.organize.mapper.merge.SysUserPostMergeMapper;
...
//        LambdaQueryWrapper<SysOrganizeRoleMerge> wrapper = new QueryWrapper<SysOrganizeRoleMerge>().lambda();
//        wrapper.eq(SysOrganizeRoleMerge::getUserId,userId);
        ...
        if (ArrayUtil.isNotEmpty(organizeIds)) {
            ...
            if (CollUtil.isNotEmpty(deptList)) {
                ...
                if (CollUtil.isNotEmpty(originalDeptList)) {
                    List<Long> originalDeptIds =
                            originalDeptList.stream().map(SysRoleDeptMerge::getDeptId).collect(Collectors.toList());
            ...
            if (CollUtil.isNotEmpty(originalPostList)) {
                List<Long> originalPostIds =
                        originalPostList.stream().map(SysRolePostMerge::getPostId).collect(Collectors.toList());
    ...
    public void editDeptRoleMerge(Long deptId, Long[] roleIds) {
        ...
        if (ArrayUtil.isNotEmpty(roleIds)) {
            ...
            if (CollUtil.isNotEmpty(originalRoleList)) {
                List<Long> originalRoleIds =
                        originalRoleList.stream().map(SysOrganizeRoleMerge::getRoleId).collect(Collectors.toList());
            ...
            if (CollUtil.isNotEmpty(roleIdList)) {
                List<SysOrganizeRoleMerge> organizeRoleMerges = roleIdList.stream().
                        map(roleId -> new SysOrganizeRoleMerge(deptId, roleId, OrganizeConstants.OrganizeType.DEPT)).collect(Collectors.toList());
    ...
    public void editUserRoleMerge(Long userId, Long[] roleIds) {
        ...
        if (ArrayUtil.isNotEmpty(roleIds)) {
            ...
            if (CollUtil.isNotEmpty(roleIdList)) {
                List<SysOrganizeRoleMerge> organizeRoleMerges = roleIdList.stream().map(roleId ->
                        new SysOrganizeRoleMerge(userId, roleId, OrganizeConstants.OrganizeType.USER)).collect(Collectors.toList());
                organizeRoleMergeMapper.insertBatch(organizeRoleMerges);
            }
        } else {
            organizeRoleMergeMapper.delete(
     ...
     *
     * @param sysRoleUserReq 查询对象
     * @return 匹配到的用户列表
     */

    @Override
    public List<SysRoleUserVO> getRoleUser(SysRoleUserReq sysRoleUserReq){
        return organizeRoleMergeMapper.getRoleUser(sysRoleUserReq);
    }
